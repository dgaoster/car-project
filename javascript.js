...
var targetInput = document.getElementById('car-mm'),
    results = document.getElementById('autocomplete-results'),
    carList = ['Toyota Camry', 'Toyota Sienna', 'Lexus ES350', 'Filler', 'Test', 'Apple', 'Banana'],
    matches = [],
    resultsCursor = 0;
...
// Define a function for checking if the input value matches any of the car names
function getMatches(inputText) {
    var matchList = [];

    for (var i = 0; i < carList.length; i++) {
        if (carList[i].toLowerCase().indexOf(inputText.toLowerCase()) != -1) {
            matchList.push(carList[i]);
        }
    }
